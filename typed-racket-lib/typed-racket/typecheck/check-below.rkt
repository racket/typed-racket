...
(define (check-below tr1 expected)
  ...
  (match* (tr1 expected)
    ...
    [((tc-result1: t1 p1 o1) (? Type? t2))
     (cond
       [(with-refinements?)
        (with-naively-extended-lexical-env [#:props (list (-is-type o1 t1)
    ...
    [((tc-result1: (? Bottom?)) _) (fix-results/bottom expected)]
    ...
    [((tc-results: tcrs _) (tc-any-results: p2))
     (define merged-prop
       (apply -and
              (map (match-lambda
    ...
    [((tc-results: tcrs1 db1) (tc-results: tcrs2 db2))
     (cond
       [(= (length tcrs1) (length tcrs2))
        (unless (andmap
                 (match-lambda**
                   [((tc-result: t1 ps1 o1) (tc-result: t2 ps2 o2))
                    (and (subtype t1 t2 o1) (prop-set-better? ps1 ps2) (object-better? o1 o2))])
                 tcrs1
                 tcrs2)
          (expected-but-got (stringify (map tc-result-t tcrs1)) (stringify (map tc-result-t tcrs2))))
        (match* (db1 db2)
          [((RestDots: dty1 dbound1) (RestDots: dty2 dbound2))
           #:when (not (and (eq? dbound1 dbound2) (subtype dty1 dty2)))
           (type-mismatch dty2 dty1 "mismatch in ... argument")]
          [(_ _) (void)])]
       [else (value-mismatch expected tr1)])
     (fix-results expected)]
