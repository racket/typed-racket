      ...
      (match f-type
        ...
        [(Function: arrs)
         ...
         (cond
           ...
           ;; if nothing matched, error or return #f
           [error-on-fail?
            (match arrs
        ...
        [(PolyRow: vars constraints (and f-ty (Function/arrs: doms _ _ #f _)))
         (define fail
           (if error-on-fail?
               (λ () (poly-fail f-stx args-stx f-type args-res
                                #:name (and (identifier? f-stx) f-stx)
                                #:expected expected))
               (λ () #f)))
         ...
         (cond [(Class? resolved-argty)
                ...
                (check args-stx
                       (subst-all substitution f-ty)
                       args-res
                       error-on-fail?)]
               [else (fail)])]
        ;; procedural structs
        [(Struct: _ _ _ (? Function? proc-ty) _ _)
         ;; the current supported proc-ty's for procedural structs take
         ;; the structure as the first argument (i.e. like a 'self' argument)
         (check #`(#,(syntax/loc f-stx dummy) . #,args-stx)
                proc-ty
                (cons (ret f-type) args-res)
                error-on-fail?)]
        ...
        [(Param: in out)
         (match argtys
           ...
           [_ (tc-error/expr
               ...
               (length argtys))])]
        [(Distinction: _ _ t)
         (check args-stx t args-res error-on-fail?)]
        ;; resolve names, polymorphic apps, mu, etc
        [(? resolvable?)
         (check args-stx (resolve-once f-type) args-res error-on-fail?)]
        ;; a union of functions can be applied if we can apply all of the elements
        [(Union: (? Bottom?) ts)
         #:when (andmap Function? ts)
         (merge-tc-results
          (for/list ([fty (in-list ts)])
            (check args-stx fty args-res error-on-fail?)))]
        [(Intersection: maybe-ftys _)
         (define results (for*/list ([fty (in-list maybe-ftys)]
                                     [res (in-value (check args-stx fty args-res #f))]
                                     #:when res)
                           res))
         (cond
           [(null? results)
